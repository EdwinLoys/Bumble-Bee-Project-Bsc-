			 ...
			 $(document).ready(function(){
                 ...
                 if (output.response=="success") {
                   swal({
                     ...
                     window.location.href = "../FRONTEND/personalisepackage.php";
                 });
               }else {
                 swal({
                   title: output.title,
		...
		<div class="payment-details-form">
			<h1>Payment Credentials</h1>
				...
				<div class="row">
					<div class="col-md-6">
						<div class="form-group">
							<input type="text" class="form-control input" id="card_ccv" data-mask="000" placeholder="CVC">
						</div>
					</div>
				</div>
				...
				<div class="row">
					<div class="col-md-6">
						<div class="form-group">
							<input type="text" class="form-control input"  data-mask="0000 0000 0000 0000" placeholder="Card Number" id="card_no" >
						</div>
					</div>
				</div>
				...
				<div class="row">
					<div class="col-md-6">
						<div class="form-group">
							<input type="text" class="form-control input" id="card_expiry" name="expiry-data"  data-mask="00 / 00"  placeholder="00 / 00" >
						</div>
					</div>
				</div>
			</form>
    ...
    <tbody>
        <tr>
              ...
              if(!empty($_SESSION['package_cart']))
              {
                ...
                $sql="SELECT * FROM packages_tbl WHERE PackageID=".$values['package_id'];
                $res=mysqli_query($conn,$sql);

                if ($row=mysqli_fetch_array($res)) {
                  echo '
                  <td><img src="../images/Package_Images/'.$values["img"].'" width="100px"></td>
                  <td> '.$row["PackageName"].'</td>
                  <td> '.$values["name"].'</td>
                  <td> '.$values["note"].'</td>
                  <td align="right">Rs '.$row["Price"].'</td>
		...
		<div class="button-placement">
			...
			<a href="javascript:history.go(-1)" class="btn btn-primary" id="cancel">Cancel</a>
